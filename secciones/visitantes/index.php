...
if(isset($_GET['txtID'])){
    ...
    $sentencia=$conexion->prepare("DELETE FROM `visitantes` WHERE Cedula=:Cedula");
...
$sentencia = $conexion->prepare("SELECT *,
    (SELECT Placa
    FROM `vehiculos`
    WHERE vehiculos.PLaca = visitantes.Vehiculo LIMIT 1) as Vehiculo,
    (SELECT Apartamento
    FROM `apartamentos`
    WHERE apartamentos.ID = visitantes.Apartamento limit 1) as Apartamento
FROM visitantes");

$sentencia->execute();
$lista_visitantes = $sentencia->fetchAll(PDO::FETCH_ASSOC);
...
<?php foreach ($lista_visitantes as $registro) { ?>
